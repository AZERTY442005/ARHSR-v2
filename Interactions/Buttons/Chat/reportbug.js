...
module.exports = {
    ...
    async execute(interaction, bot, config) {
        ...
        if (ReportBugCooldown.has(interaction.user.id)) { // Command Cooldown
            const Embed = new MessageEmbed()
            .setAuthor({name:`${interaction.user.tag}`, iconURL:interaction.user.displayAvatarURL({dynamic:true})})
            .setDescription(`:alarm_clock: ${MessageLanguage("WaitReportBug", bot).replace("{delay}", CooldownDelay/60)}`)
            .setColor("ORANGE")
            ...
            const Embed = new MessageEmbed()
            .setDescription(":white_check_mark: Bug rapporté. Merci de votre aide")
            .setAuthor({name:`${interaction.user.tag}`, iconURL:interaction.user.displayAvatarURL({dynamic:true})})
            .setColor("GREEN")
            ...
            // UPDATE COOLDOWN
            ReportBugCooldown.add(interaction.user.id)
            setTimeout(() => {
                ReportBugCooldown.delete(interaction.user.id)
